...
def create_mock_objects(user_id: int, file_hash: str, filename: str, document_content: str):
    ...
    class MockUserFile:
        def __init__(self, file_hash: str, filename: str, content_size: int):
            self.id = 1  # Mock ID
    ...
    user_file = MockUserFile(file_hash, filename, len(document_content))
    return user, user_file


async def get_document_content(minio_service: MinIOService, file_hash: str) -> str:
    """Get document content from MinIO storage."""
    if not minio_service.file_exists(file_hash):
        raise HTTPException(status_code=404, detail="Document not found in storage")
    
    file_stream = minio_service.get_file_stream(file_hash)
...
async def process_document(
    ...
    try:
        ...
        document_content = await get_document_content(minio_service, request.file_hash)
        
        # Create mock user and user_file objects for processing
        # In a real implementation, you'd fetch these from the database
        user, user_file = create_mock_objects(
            request.user_id, request.file_hash, request.filename, document_content
        )
